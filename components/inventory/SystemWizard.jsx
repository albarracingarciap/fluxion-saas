...
// Componente compartido · Alta y Edición · 7 pasos
// ─────────────────────────────────────────────────────────────
...
export const INIT = {
  ...
  status:"",deployedAt:"",description:"",technicalDesc:"",
  // Step 2
  intendedUse:"",prohibitedUses:"",outputTypes:[],
  fullyAutomated:null,interactsPersons:false,targetUsers:[],usageScale:"",geoScope:[],
  // Step 3
...
function LivePreview({f,step,analysing,mode}){
  const fWithOutput = useMemo(()=>({...f,outputType:f.outputTypes?.[0]??f.outputType??""}),[f]);
  ...
  const outObj = OUTPUTS.find(o=>o.v===(f.outputTypes?.[0]??f.outputType));
        ...
        {step===7&&f.name&&f.domain&&cls&&(()=>{
          ...
          return(
            <div className="comp-banner fi">
              ...
              <div className="comp-ttl">
                {gaps>0
                  ? `${isEdit?"Guardar":"Registrar"} con ${gaps} gap${gaps>1?"s":""} crítico${gaps>1?"s":""}`
                  : (isEdit ? "✓ Listo para guardar cambios" : "✓ Listo para registrar")
                }
              </div>
              <div className="comp-sub">
                {gaps>0
                  ? `El sistema quedará ${isEdit?"actualizado":"en el inventario"}. El agente marcará los gaps críticos para resolución prioritaria.`
                  : (isEdit
                      ? "Los cambios actualizarán el sistema en inventario y recalcularán AI Act e ISO 42001."
                      : "El sistema quedará en el inventario y el agente iniciará la evaluación de conformidad formal.")
                }
              </div>
            </div>
          );
...
function Step2({f,set,cbToggle}){return(<>
  ...
  <div className="field">
    <Lbl tags={["aiact"]}>¿Las decisiones o recomendaciones son totalmente automáticas?</Lbl>
    <Opt options={[
...
function Step3({f,set}){return(<>
  ...
  <Toggle on={f.biometric} onToggle={()=>set("biometric",!f.biometric)}
    label="Procesa datos biométricos" sub="Rostro, voz, huella dactilar, marcha, patrones de comportamiento, verificación de identidad → Anexo III §1"/>
  ...
  <div className="field">
    ...
    {f.dpiaCompleted==="no"&&f.specialCategories?.length>0&&(
      <div className="warn re" style={{marginTop:8}}>
        ...
        <span><strong>DPIA obligatoria.</strong> Has seleccionado categorías especiales (Art. 9) y la DPIA no está realizada. El Art. 35 RGPD la exige antes del inicio del tratamiento.</span>
      </div>
    )}
  </div>
  <Toggle on={f.intlDataTransfers??false} onToggle={()=>set("intlDataTransfers",!f.intlDataTransfers)}
...
function Step6({f,set}){return(<>
  ...
  <div className="field">
    ...
    <div className="tag-wrap" onClick={()=>document.getElementById("prov-inp").focus()}>
      {(Array.isArray(f.criticalProviders)?f.criticalProviders:[]).map((p,i)=>(
      ...
      <input id="prov-inp" className="tag-inp" placeholder="Añadir proveedor y pulsar Enter (ej. Azure, AWS, OpenAI...)"
        onKeyDown={e=>{if(e.key==="Enter"&&e.target.value.trim()){set("criticalProviders",[...(Array.isArray(f.criticalProviders)?f.criticalProviders:[]),e.target.value.trim()]);e.target.value=""}}}/>
    </div>
...
/**
 * SystemWizard — Wizard compartido para alta y edición de sistemas IA.
 *
 * Props:
 *   mode        'create' | 'edit'  (default: 'create')
 *   systemId    string             ID del sistema (requerido en modo edit)
 *   systemName  string             Nombre del sistema (para el breadcrumb en edit)
 *   initialData object             Datos iniciales del formulario (edit: datos del sistema)
 *   initialTags string[]           Tags iniciales (edit: tags del sistema)
 *   onSave      async (payload) => { error?, success?, id? }
 *               Función que guarda los datos. Puede ser saveAISystem o updateAISystem.
 */
export default function SystemWizard({
  ...
  const [f, setF] = useState(() => {
    ...
    // Normalize initialData to wizard format
    return {
      ...INIT,
      ...initialData,
      // Ensure outputTypes is always an array
      outputTypes: Array.isArray(initialData.outputTypes)
        ? initialData.outputTypes
        : initialData.outputType
          ? [initialData.outputType]
          : [],
      // Ensure criticalProviders is always an array
      criticalProviders: Array.isArray(initialData.criticalProviders)
        ? initialData.criticalProviders
        : initialData.criticalProviders
          ? String(initialData.criticalProviders).split(', ').filter(Boolean)
          : [],
      // Ensure array fields are arrays
      legalBasesArt9: initialData.legalBasesArt9 ?? [],
      intlDataTransfers: initialData.intlDataTransfers ?? false,
      ossModelName: initialData.ossModelName ?? '',
      ossLicense: initialData.ossLicense ?? '',
      hasExplainability: initialData.hasExplainability ?? null,
      lastReviewDate: initialData.lastReviewDate ?? '',
    };
  });
  ...
  const handleSave = async () => {
    ...
    // Normalize to the format both saveAISystem and updateAISystem expect
    const payload = {
      ...f,
      tags,
      // Normalize outputTypes[] → outputType (single string)
      outputType: f.outputTypes?.[0] ?? "",
      // Normalize criticalProviders[] → string
      criticalProviders: Array.isArray(f.criticalProviders)
        ? f.criticalProviders.join(", ")
        : (f.criticalProviders || null),
    ...
    if (result?.error) {
      ...
      router.push(mode === 'create' ? '/inventario' : `/inventario/${systemId}`);
    }
  };
  ...
  const breadcrumbLabel = isEdit
    ? `Editar${systemName ? ` ${systemName}` : ' sistema'}`
    : 'Registrar sistema';
  const saveLabel = isEdit ? '✓ Guardar cambios' : '✓ Registrar sistema';
    ...
    <div className="flex flex-col min-h-screen bg-ltbg text-ltt">
      ...
      <div className="h-14 bg-ltcard border-b border-ltb flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-2 font-plex text-[12px] text-lttm">
          ...
          <span className="text-ltt font-medium">{breadcrumbLabel}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden p-6 max-w-[1440px] mx-auto w-full">
        <div className="wiz-wrapper" style={{ height: '100%' }}>
          ...
          <div className="wiz">
            ...
            <div className="form-col">
                ...
                <div className="step-track">
                  {STEPS.map((s, i) => (
                    <div key={s.id} style={{ display: "flex", alignItems: "flex-start", flex: i < STEPS.length - 1 ? 1 : 0 }}>
                      <div
                        className={`step-node${step > s.id ? " done" : step === s.id ? " active" : ""}`}
              ...
              <div className="step-nav">
                <div className="step-prog">
                  {step < 7 ? `Siguiente: ${STEPS[step]?.lbl || ""}` : "Listo para guardar"}
                </div>
                <div className="step-btns">
                  ...
                  {step < 7 ? (
                    ...
                    <>
                      {saveError && (
                        <span style={{ fontSize: 11.5, color: 'var(--re)', fontFamily: "'IBM Plex Mono',monospace", maxWidth: 200 }}>
                          {saveError}
                        </span>
                      )}
                      <button
            ...
            {/* PREVIEW */}
            <LivePreview f={{ ...f, tags }} step={step} analysing={analysing} mode={mode} />
          </div>
        </div>
      </div>
    </div>
