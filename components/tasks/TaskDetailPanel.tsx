...
import { X, Edit2, Save, Trash2, Loader2, ExternalLink, XCircle, Plus } from 'lucide-react'
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: show ? 'rgba(0,0,0,0.18)' : 'transparent',
      ...
      {/* Panel */}
      <div
        className="fixed right-0 top-0 bottom-0 z-50 w-[440px] max-w-full bg-ltcard border-l border-ltb shadow-[-8px_0_40px_rgba(0,0,0,0.12)] flex flex-col"
        style={{
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-ltb bg-ltcard2 shrink-0">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`font-plex text-[9px] uppercase tracking-[0.5px] px-2 py-0.5 rounded-full border ${STATUS_STYLES[task.status]}`}>
              {TASK_STATUS_LABELS[task.status]}
            </span>
            <span className="font-plex text-[9px] uppercase tracking-[0.5px] text-lttm bg-ltbg border border-ltb px-2 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-1.5">
            {!editing && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-ltt2 hover:text-ltt hover:bg-ltbg border border-transparent hover:border-ltb transition-all font-sora text-[12px]"
              >
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-5">
          ...
          <div>
            {editing ? (
              ...
              <h2 className="font-sora text-[16px] font-semibold text-ltt leading-snug">{task.title}</h2>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {editing ? (
                ...
                <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border font-plex text-[10px] uppercase tracking-[0.5px] ${STATUS_STYLES[task.status]}`}>
                  {TASK_STATUS_LABELS[task.status]}
                </span>
              )}
            </div>
            <div>
              ...
              {editing ? (
                ...
                <span className={`inline-flex items-center px-2 py-0.5 rounded-full border font-plex text-[10px] uppercase tracking-[0.5px] ${PRIORITY_STYLES[task.priority]}`}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {editing ? (
                ...
                <p className={`font-sora text-[13px] ${task.assignee_name ? 'text-ltt' : 'text-lttm italic'}`}>
            ...
            <div>
              ...
              {editing ? (
                ...
                <p className={`font-sora text-[13px] ${overdue ? 'text-re font-medium' : task.due_date ? 'text-ltt' : 'text-lttm italic'}`}>
                  {task.due_date ? formatDate(task.due_date) : 'Sin fecha'}
                  {overdue && <span className="ml-1.5 font-plex text-[10px] uppercase tracking-[0.5px]">· Vencida</span>}
                </p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            {editing ? (
              ...
              <p className={`font-sora text-[13px] leading-relaxed whitespace-pre-wrap ${task.description ? 'text-ltt2' : 'text-lttm italic'}`}>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {(editing ? tags : task.tags).map(tag => (
                <span
                  key={tag}
                  className="inline-flex items-center gap-1 font-plex text-[9px] uppercase tracking-[0.5px] px-2 py-0.5 bg-cyan-dim text-brand-cyan border border-cyan-border rounded-full"
                >
                  {tag}
                  {editing && (
                    <button
                      ...
                      className="hover:text-re transition-colors ml-0.5"
                    >
                ...
                <div className="flex items-center gap-1">
                  ...
                  {tagInput && (
                    <button
                      ...
                      className="w-5 h-5 flex items-center justify-center rounded-full bg-brand-cyan text-white shrink-0"
                    >
              ...
              {!editing && task.tags.length === 0 && (
                <p className="font-sora text-[13px] text-lttm italic">Sin etiquetas</p>
              )}
            </div>
          </div>
          ...
          {sourceHref && (
            <div>
              ...
              <Link
                href={sourceHref}
                className="inline-flex items-center gap-1.5 font-sora text-[12px] text-brand-cyan hover:underline"
              >
          ...
          {/* Metadata */}
          <div className="pt-2 border-t border-ltb flex flex-col gap-2">
            <div>
            ...
            {task.completed_at && (
              <div>
                <p className="font-plex text-[9px] uppercase tracking-[0.7px] text-gr">Completada</p>
        ...
        {/* Footer */}
        {(editing || task.source_type === 'manual') && (
            ...
            {editing ? (
              <>
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-gradient-to-br from-[#00adef] to-[#33c3f5] text-white rounded-[8px] font-sora text-[12px] font-medium disabled:opacity-50 shadow-[0_2px_8px_rgba(0,173,239,0.3)] transition-opacity"
                >
                ...
                <button
                  onClick={cancelEdit}
                  className="px-4 py-2 font-sora text-[12px] text-ltt2 hover:text-ltt transition-colors"
                >
                  Cancelar
                </button>
              </>
              ...
              <>
                {confirmDelete ? (
                  <>
                    <span className="font-sora text-[12px] text-re">¿Eliminar esta tarea?</span>
                    <button
                    ...
                    <button
                      ...
                      className="font-sora text-[12px] text-ltt2 hover:text-ltt transition-colors"
                    >
                      Cancelar
                    </button>
                  </>
