...
export default async function EditSystemPage({ params }: { params: { id: string } }) {
  ...
  const initialForm = {
    ...
    outputTypes: system.output_type ? [system.output_type] : [],
    ...
    legalBasesArt9: system.legal_bases_art9 ?? [],
    intlDataTransfers: system.intl_data_transfers ?? false,
    ...
    ossModelName: system.oss_model_name ?? '',
    ossLicense: system.oss_license ?? '',
    hasExplainability: system.has_explainability ?? null,
    frameworks: system.frameworks ?? '',
    origin: system.provider_origin ?? '',
    valueChainRoles: system.value_chain_roles ?? [],
    ...
    criticalProviders: system.critical_providers
      ? system.critical_providers.split(', ').filter(Boolean)
      : [],
    reviewFrequency: system.review_frequency ?? '',
    lastReviewDate: system.last_review_date ?? '',
