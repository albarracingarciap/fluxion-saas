...
function EmptyDashboard({ organizationName, firstFocus }: { organizationName: string; firstFocus?: string }) {
  const focusKey = (firstFocus && firstFocus in FOCUS_CONFIG ? firstFocus : 'inventory') as FocusKey
  const focus = FOCUS_CONFIG[focusKey]
      ...
      <div className="bg-ltcard border border-ltb rounded-[14px] shadow-[0_2px_12px_rgba(0,74,173,0.04)] overflow-hidden">
        <div className="bg-ltcard2 px-6 py-4 border-b border-ltb flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-[9px] bg-cyan-dim2 border border-cyan-border flex items-center justify-center shrink-0">
              <FocusIcon size={16} className="text-brand-cyan" />
            </div>
            <div>
              <p className="font-plex text-[10px] uppercase tracking-[0.9px] text-lttm">Tu prioridad de despliegue</p>
              <p className="font-sora text-[13px] font-semibold text-ltt">{focus.label}</p>
            </div>
          </div>
          <span className="font-plex text-[10px] uppercase tracking-[0.8px] px-2 py-1 rounded-full bg-cyan-dim text-brand-cyan border border-cyan-border">
            Primeros pasos
        ...
        <div className="p-6 grid gap-4 md:grid-cols-[1fr_1fr_1fr_auto] md:items-center">
