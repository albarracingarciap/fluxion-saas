    ...
    <div className="prev-col">
      ...
      <div className="prev-body">
        ...
        {step===7&&f.name&&f.domain&&cls&&(
          ...
          <div className="comp-ttl">Listo para guardar cambios</div>
          <div className="comp-sub">Los cambios actualizarán el sistema en inventario y recalcularán AI Act e ISO 42001 en servidor.</div>
          </div>
        )}
      </div>
    </div>
...
export default function EditSystemClient({ systemId, systemName, initialForm, initialTags }){
  ...
  const [f,setF]=useState(initialForm ?? INIT);
  ...
  const handleRegister = async () => {
    ...
    const result = await updateAISystem(payload);
    setSaving(false);
    if(result?.error) {
      setSaveError(result.error);
    } else {
      router.push(`/inventario/${systemId}`);
    }
  };
    ...
    <div className="flex flex-col min-h-screen bg-ltbg text-ltt">
      ...
      <div className="h-14 bg-ltcard border-b border-ltb flex items-center justify-between px-6 shrink-0">
        <div className="flex items-center gap-2 font-plex text-[12px] text-lttm">
          ...
          <span className="text-ltt font-medium">{systemName ? `Editar ${systemName}` : 'Editar sistema'}</span>
        </div>
