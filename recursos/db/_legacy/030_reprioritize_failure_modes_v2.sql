-- ============================================================================
-- FLUXION — Repriorización v2 de modos de fallo activados
-- ============================================================================
--
-- Objetivo:
--   Endurecer el paso a prioritized para reducir la cola FMEA manual:
--   - critical -> prioritized
--   - high -> prioritized solo con señal sensible o dimensión operativa fuerte
--   - ROI nunca por score solo
--   - gobernanza con S_default < 8 solo entra con señal sensible / AI Act alto
--   - cuota blanda: min(80, ceil(activated_count * 0.25))
--   - la cuota nunca expulsa overrides duros ni critical
...
BEGIN
  ...
  v_is_high_candidate := (
    v_level = 'high'
    AND (
      v_dimension IN ('seguridad', 'legal_b', 'etica', 'tecnica')
      OR v_has_sensitive_signal
    )
    AND v_dimension <> 'roi'
    AND NOT (
      v_dimension = 'gobernanza'
      AND p_s_default < 8
      AND NOT v_has_sensitive_signal
      AND v_aiact_level NOT IN ('high', 'prohibited')
    )
  );
...
SET
  ...
  priority_notes = CASE
    WHEN final_statuses.final_status = 'prioritized'
      THEN 'Prioridad recalculada por reglas v2: entra en la cola prioritaria de revisión.'
    ELSE 'Prioridad recalculada por reglas v2: queda en observación para este ciclo.'
  END,
