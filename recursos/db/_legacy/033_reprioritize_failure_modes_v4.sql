-- ============================================================================
-- FLUXION — Repriorización v4 de modos de fallo activados
-- ============================================================================
--
-- Ajustes v4 definitivos de esta iteración:
--   - critical se mantiene en >= 75
--   - overrides duros solo:
--       * S_default >= 8
--       * biometric + affects_persons
--       * has_minors + dimensión sensible
--   - se eliminan como override:
--       * aiact high/prohibited + dimensión sensible
--       * vulnerable_groups + dimensión sensible
--   - canal high:
--       * seguridad / tecnica -> permitido con S_default >= 7
--       * etica / legal_b -> solo con señal humana fuerte y S_default >= 7
--       * gobernanza fuera del canal high
...
SET
  ...
  priority_notes = CASE
    WHEN final_statuses.final_status = 'prioritized'
      THEN 'Prioridad recalculada por reglas v4: entra en la cola prioritaria de revisión.'
    ELSE 'Prioridad recalculada por reglas v4: queda en observación para este ciclo.'
  END,
