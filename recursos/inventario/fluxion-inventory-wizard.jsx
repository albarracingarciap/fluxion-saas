...
const CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  :root{
    ...
    --dt:#e8f0fe;--dt2:#7a9cc4;--dtm:#3d5a82;--dtd:#233249;
  ...
  body{font-family:'Sora',sans-serif;background:var(--lt);color:var(--lt1)}
  ::-webkit-scrollbar{width:4px}
  ::-webkit-scrollbar-thumb{background:var(--lbl);border-radius:2px}
  ...
  /* WIZARD */
  .wiz{flex:1;overflow:hidden;display:grid;grid-template-columns:1fr 370px}
  .form-col{overflow-y:auto;display:flex;flex-direction:column;background:var(--lt)}
  ...
  .iso-dot.ok{background:var(--gr)} .iso-dot.no{background:var(--re)} .iso-dot.na{background:var(--dtm)}
...
function classifyAIAct(f) {
  if (f.isAISystem === false) return null;
  if (f.prohibitedPractice) return {
    level:"prohibited",label:"⛔ Práctica Prohibida",basis:"Art. 5 — Prohibición absoluta",
    reason:"El sistema tiene características que corresponden a prácticas prohibidas. NO puede desplegarse en la UE.",
    obls:["⛔ Despliegue prohibido en la UE","Retirada o rediseño obligatorio","Art. 5 — Sistemas de manipulación, scoring social, biometría remota en tiempo real"]
  };
  if (f.isGPAI) return {
    level:"gpai",label:"Modelo GPAI",basis:"Art. 51-53 — IA de uso general",
    reason:"Modelo de propósito general. Obligaciones de transparencia, documentación y —si hay riesgo sistémico— Art. 55.",
    obls:["Art. 53 — Documentación técnica","Art. 53 — Política de copyright","Art. 53 — Resumen de datos de entrenamiento","Art. 55 — Riesgo sistémico (si aplica)"]
  };
  const hi = f.affectsPersons;
  if (f.biometric && hi) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §1 — Biometría",
    reason:"Sistema que procesa datos biométricos de personas físicas para identificación remota o categorización.",
    obls:ART_HIGH
  };
  if (f.criticalInfra) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §2 — Infraestructura crítica",
    reason:"Sistema que gestiona componentes de infraestructura crítica (energía, agua, transporte...).",
    obls:ART_HIGH
  };
  if (f.domain==="educacion"&&hi&&["decision","clasificacion","prediccion"].includes(f.outputType)) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §3 — Educación y formación",
    reason:"Sistema que determina el acceso a formación, evalúa a estudiantes o afecta su trayectoria educativa.",
    obls:ART_HIGH
  };
  if ((f.domain==="rrhh")&&hi&&["decision","clasificacion","prediccion"].includes(f.outputType)) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §4 — Empleo y RRHH",
    reason:"Sistema que interviene en selección, evaluación, promoción o despido de personas trabajadoras.",
    obls:ART_HIGH
  };
  if ((f.domain==="credito"||f.domain==="finanzas"||f.domain==="seguros")&&hi&&["decision","prediccion","clasificacion"].includes(f.outputType)) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §5 — Servicios financieros esenciales",
    reason:"Sistema que evalúa solvencia, asigna crédito o determina condiciones de seguros para personas.",
    obls:ART_HIGH
  };
  if (f.domain==="salud"&&hi) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §5 / Anexo I — Salud",
  ...
  if ((f.domain==="seguridad"||f.domain==="justicia")&&hi) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §6/8 — Seguridad / Justicia",
    reason:"Sistema usado en seguridad pública, aplicación de la ley o administración de justicia.",
    obls:ART_HIGH
  };
  if (f.domain==="migracion"&&hi) return {
    level:"high",label:"Alto Riesgo",basis:"Anexo III §7 — Migración y fronteras",
    reason:"Sistema que afecta decisiones sobre visados, asilo o control fronterizo.",
    obls:ART_HIGH
  };
  if (f.outputType==="generacion") return {
    level:"limited",label:"Riesgo Limitado",basis:"Art. 50.2/50.4 — Contenido sintético",
    reason:"Sistema que genera texto, imágenes u otro contenido sintético. Obligación de disclosure y marcado.",
    obls:["Art. 50.2 — Marcar contenido como generado por IA","Art. 50.4 — Deep fakes: consentimiento o disclosure claro"]
  };
  if (f.interactsPersons) return {
    level:"limited",label:"Riesgo Limitado",basis:"Art. 50.1 — Interacción con personas",
  ...
  if (f.isAISystem===null||!f.outputType) return null;
  return {
    level:"minimal",label:"Riesgo Mínimo",basis:"Sin categoría específica AI Act",
...
const ART_HIGH = [
  "Art. 9 — Sistema de gestión de riesgos","Art. 10 — Gobernanza de datos de entrenamiento",
  "Art. 11 — Documentación técnica completa","Art. 12 — Registro automático de actividad (logs)",
  "Art. 13 — Transparencia e información a usuarios","Art. 14 — Supervisión humana obligatoria",
  "Art. 15 — Precisión, robustez y ciberseguridad","Art. 16 — Obligaciones del proveedor",
  "Art. 43 — Evaluación de conformidad","Art. 71 — Registro en EU AI Office",
];
...
function calcISO(f) {
  const checks = [
    {k:"aiOwner",lbl:"Responsable IA designado",pts:12},
    {k:"hasTechDoc",lbl:"Documentación técnica",pts:12},
    {k:"hasLogging",lbl:"Logging de actividad",pts:10},
    {k:"humanOversight",lbl:"Supervisión humana",pts:12},
    {k:"hasRiskAssessment",lbl:"Evaluación de riesgos",pts:14},
    {k:"dpoInvolved",lbl:"DPO involucrado",pts:8},
    {k:"reviewFrequency",lbl:"Periodicidad de revisión",pts:10},
    {k:"incidentContact",lbl:"Contacto de incidentes",pts:7},
    {k:"dpiaCompleted",lbl:"DPIA completada",pts:8},
    {k:"hasAdversarialTest",lbl:"Test de robustez",pts:7},
  ];
  let total=0,max=0;
  const results = checks.map(c=>{
    const val = f[c.k];
    const ok = val===true||val==="si"||(typeof val==="string"&&val.trim().length>1);
    const na = val===null||val===undefined||val==="";
    if(!na) max+=c.pts;
    if(ok) total+=c.pts;
    return{...c,ok,na};
  });
  const score = max>0?Math.round((total/checks.reduce((a,c)=>a+c.pts,0))*100):0;
...
function Logo(){return(
  <svg viewBox="0 0 240 58" xmlns="http://www.w3.org/2000/svg" style={{width:"100%",maxWidth:174,height:42,display:"block"}}>
    <defs>
      <linearGradient id="lg1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#00c8f8"/><stop offset="100%" stopColor="#00adef"/></linearGradient>
      <linearGradient id="lg2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#3871c1"/><stop offset="100%" stopColor="#004aad"/></linearGradient>
      <linearGradient id="lg3" x1="0%" y1="100%" x2="100%" y2="0%"><stop offset="0%" stopColor="#004aad"/><stop offset="100%" stopColor="#3871c1"/></linearGradient>
    </defs>
    <ellipse cx="31" cy="32" rx="14" ry="11" fill="url(#lg1)"/>
    <ellipse cx="20" cy="30" rx="10" ry="8.5" fill="url(#lg1)"/>
    <ellipse cx="42" cy="30" rx="10" ry="8.5" fill="url(#lg1)"/>
    <ellipse cx="23" cy="22" rx="9" ry="7.5" fill="url(#lg1)"/>
    <ellipse cx="37" cy="21" rx="8" ry="6.5" fill="url(#lg1)"/>
    <ellipse cx="30" cy="18" rx="7" ry="5.5" fill="url(#lg1)"/>
    <rect x="11" y="30" width="40" height="13" fill="url(#lg1)" rx="2"/>
    <path d="M11 42 C11 35 17 31 24 34 C28 36 30 38.5 31 39.5 C30 40.5 28 43 24 45 C17 48 11 49 11 42 Z" fill="url(#lg3)"/>
    <path d="M51 42 C51 35 45 31 38 34 C34 36 32 38.5 31 39.5 C32 40.5 34 43 38 45 C45 48 51 49 51 42 Z" fill="url(#lg3)"/>
    <ellipse cx="31" cy="39.5" rx="3" ry="3" fill="url(#lg1)"/>
    <text x="62" y="38" fontFamily="'Sora',sans-serif" fontWeight="700" fontSize="23" letterSpacing="0.5" fill="url(#lg2)">FLUXION</text>
    <text x="63" y="50" fontFamily="'IBM Plex Mono',monospace" fontWeight="400" fontSize="8" letterSpacing="2.5" fill="#3871c1" opacity="0.65">AI GOVERNANCE</text>
  </svg>
)}
...
const INIT = {
  // Step 1
  name:"",version:"1.0.0",internalId:"",domain:"",subdomain:"",
          ...
          <div className="sys-card">
            ...
            <div className="sys-card-body">
              {[
                ...
                {k:"Sup. humana",v:f.humanOversight===true?"Sí":f.humanOversight===false?"No":null},
              ...
              <div className="iso-items">
                {iso.checks.filter(c=>!c.na).map((c,i)=>(
                  <div key={i} className="iso-item">
                    <div className={`iso-dot ${c.ok?"ok":"no"}`}/>
                    {c.ok?"✓":"-"} {c.lbl}
                  </div>
                ))}
...
function Step4({f,set,cbToggle}){return(<>
  ...
  <div className="field">
    ...
    <Opt options={[
      {v:true,l:"Sí, completada",i:"✅"},{v:"proceso",l:"En proceso",i:"🔄"},{v:false,l:"No realizada",i:"❌"}
    ]} value={f.dpiaCompleted} onChange={v=>set("dpiaCompleted",v==="true"||v===true)} grid="c3"/>
...
function Step7({f,set}){return(<>
  ...
  <div className="field">
    ...
    <Opt options={[
      {v:true,l:"Sí, logging activo",i:"📋",d:"Registra inputs, outputs, decisiones y eventos relevantes"},
      {v:"parcial",l:"Parcial",i:"🟡",d:"Logging básico pero no cubre todos los requisitos"},
      {v:false,l:"No hay logging",i:"❌",d:"Sin registro de actividad del sistema"},
    ]} value={f.hasLogging} onChange={v=>set("hasLogging",v==="true"||v===true)} grid="c3"/>
  </div>
  <div className="field">
    <Lbl req tags={["aiact","iso"]}>Supervisión humana (Art. 14)</Lbl>
    <Opt options={[
      {v:true,l:"Sí, supervisión activa",i:"👤",d:"Humanos pueden intervenir, pausar o revertir"},
      {v:"parcial",l:"Supervisión parcial",i:"🔍",d:"Solo en casos de umbral o muestreo"},
      {v:false,l:"Sin supervisión humana",i:"⚡",d:"El sistema opera completamente autónomo"},
    ]} value={f.humanOversight} onChange={v=>set("humanOversight",v==="true"||v===true)} grid="c3"/>
    {f.humanOversight!==true&&f.affectsPersons&&<div className="warn or" style={{marginTop:10}}><span className="warn-ico">⚠️</span><span>La supervisión humana es <strong>obligatoria</strong> para sistemas de alto riesgo que afectan a personas.</span></div>}
  </div>
  {(f.humanOversight===true||f.humanOversight==="parcial")&&(
    <div className="field">
  ...
  <div className="field">
    ...
    <Opt options={[
      {v:true,l:"Sí, completada",i:"✅"},{v:"proceso",l:"En proceso",i:"🔄"},{v:false,l:"Pendiente",i:"❌"}
    ]} value={f.hasRiskAssessment} onChange={v=>set("hasRiskAssessment",v==="true"||v===true)} grid="c3"/>
  </div>
  {f.hasRiskAssessment&&(
    <div className="field-row fr2">
      <div className="field">
...
export default function App(){
  ...
  const NAV_ITEMS=[
    {ico:"◉",lbl:"Dashboard"},{ico:"⊞",lbl:"Inventario IA",on:true},
    {ico:"⚖",lbl:"AI Act"},{ico:"⬡",lbl:"Gap Analysis",badge:"11"},
    {ico:"◫",lbl:"Evidencias"},{ico:"⊹",lbl:"Roadmap"},{ico:"▤",lbl:"Reporting"},
  ];
  ...
  return(<>
    ...
    <div className="layout">
      ...
      {/* SIDEBAR */}
      <aside className="sb">
        <div className="logo-wrap"><div className="logo-card"><Logo/></div></div>
        <nav className="nav">
          <div className="nav-g">Proceso</div>
          {NAV_ITEMS.map((n,i)=>(
            <div key={i} className={`nav-i${n.on?" on":""}`}>
              <span className="nav-ico">{n.ico}</span>
              <span>{n.lbl}</span>
              {n.badge&&<span className="nav-badge">{n.badge}</span>}
            </div>
          ))}
          <div className="nav-g" style={{marginTop:12}}>Configuración</div>
          {[{ico:"⚙",lbl:"Ajustes"},{ico:"👥",lbl:"Usuarios"},{ico:"💳",lbl:"Facturación"}].map((n,i)=>(
            <div key={i} className="nav-i"><span className="nav-ico">{n.ico}</span><span>{n.lbl}</span></div>
          ))}
        </nav>
        <div className="sb-foot">
          <div className="org-chip">
            <div className="org-av">BI</div>
            <div><div className="org-nm">Banco Iberia</div><div className="org-rl">DPO · Professional + Banca</div></div>
          </div>
        </div>
      </aside>
      ...
      <div className="main">
        <div className="topbar">
          <div className="bc">
            <a>Inventario IA</a>
            <span style={{color:"var(--dtm)"}}>›</span>
            <span className="cur">Registrar nuevo sistema</span>
          </div>
          <div className="tbr">
            <button className="btn btn-g">✕ Cancelar</button>
          </div>
        </div>
            ...
            <div className="step-nav">
              ...
              <div className="step-btns">
                ...
                {step<7?(
                  ...
                  <button className="btn btn-te">✓ Registrar sistema</button>
                )}
              </div>
            </div>
          </div>
